...
| `align.py` | alias suggester — nearest canonical neighbour per unmatched LLM entity (embedding endpoint or difflib; KARMA ρ cutoff: below ρ → propose-only merge, above → new entity). CLI recomputes on an existing snapshot; `--stats` summarizes the distance distribution (histogram + ρ-boundary pairs) for ρ tuning, and `--stats --rho X` previews a cutoff without re-embedding |
